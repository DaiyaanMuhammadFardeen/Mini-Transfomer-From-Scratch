...
def main(args):
    ...
    def build_class_weights(tgt_vocab, device, alpha=0.5):
        """
        ...
        if hasattr(tgt_vocab, 'word_freqs') and tgt_vocab.word_freqs:
            ...
            for word, idx in tgt_vocab.stoi.items():
                ...
                weights[idx] = (total / (freq * vocab_size)) ** alpha

        # Always give zero weight to padding
        weights[tgt_vocab.stoi['<PAD>']] = 0.0
        return weights
    
    class_weights = build_class_weights(tgt_vocab, device)
