...
class CodeDiffDataset(Dataset):
    ...
    def __init__(self, tokenized_dir: str, src_vocab=None, split='train',
        ...
        src_vocab_size    = self.meta.get("src_vocab_size", 0)

        # Fix int16 overflow: if vocab > 32767, force int32 even if meta says int16
        if dtype_str == "int16" and src_vocab_size > 32767:
            print(f"[WARNING] src vocab ({src_vocab_size}) > int16 range (32767), forcing int32 read")
            self.dtype = np.int32
        else:
